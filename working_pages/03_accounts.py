...
# Get accounts early for duplicate checking
    ...
    if not groups:
        ...
        with st.form(form_key):
            ...
            if st.form_submit_button("💾 Save Account", use_container_width=True):
                ...
                if not clean_name or not selected_group:
                    st.error("⚠️ Name and Group are required.")
                elif clean_name.lower() in existing_names:
                    # IMMEDIATE USER INDICATION
                    st.error(f"❌ Duplicate Name: An account named '{clean_name}' already exists.")
                else:
                    try:
                        add_account(clean_name, group_map[selected_group], phone.strip(), address.strip())
                        st.session_state.acc_form_version += 1
                        st.toast(f"✅ {clean_name} added!")
                        st.rerun()
...
# Existing Accounts
# -------------------------------
with st.expander("📋 View/Edit Existing Accounts", expanded=False):
    ...
    with col_h3:
        show_inactive = st.toggle("Show Inactive", value=True)
    ...
    if not accounts:
        ...
        filtered_accounts = [
            ...
            and (show_inactive or a['is_active'] == 1)
        ]

        if not filtered_accounts:
            st.warning(f"No accounts match your search.")
        else:
            for a in filtered_accounts:
                ...
                with st.container(border=True):
                    ...
                    with c4.popover("⚙️ Manage", use_container_width=True):
                        st.markdown("### 📝 Edit Account")
                        edit_name = st.text_input("Account Name", value=a["name"], key=f"en_{a['id']}")
                        
                        # LIVE VALIDATION IN EDIT
                        if edit_name.strip().lower() in existing_names and edit_name.strip().lower() != a['name'].lower():
                            st.warning(f"⚠️ Name '{edit_name}' is already used by another account.")
                            can_update = False
                        else:
                            can_update = True
                        ...
                        with btn_col1:
                            # Disable button if name is a duplicate
                            if st.button("💾 Update", key=f"ub_{a['id']}", disabled=not (confirm_upd and can_update), on_click=clear_form_key, args=(upd_check_key,)):
                                try:
                                    update_account(a["id"], edit_name.strip(), group_map[edit_group], edit_phone.strip(), edit_addr.strip())
                                    st.toast("✅ Updated!")
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"❌ {e}")
                        ...
                        if can_delete_account(a["id"]):
                            ...
                            if st.button("❌ Delete Permanently", key=f"del_btn_{a['id']}", disabled=not confirm_del, type="primary", on_click=clear_form_key, args=(del_check_key,), use_container_width=True):
                                delete_account(a["id"])
                                st.toast("🗑️ Account deleted")
                                st.rerun()
                        else:
                            st.info("🔒 Linked to data. Cannot delete.")